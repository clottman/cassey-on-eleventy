...
I wanted a script I could run from the editor console of `cassey-til.glitch.me`, that would hit a [Netlify webhook](https://docs.netlify.com/configure-builds/build-hooks/) to rebuild [cassey.dev](https://cassey.dev). It's important that only I can run the script, though, so I can't just put the URL right in package.json where anyone can see it. 

I know that on Glitch, .env is a safe place to keep secrets. So, I stored my webhook URL there, like `NETLIFY_BUILD_URL='secret'`. 
...
Gonna run it as soon as I finish this post, so that this post will appear on both `cassey-til.glitch.me`, and [cassey.dev/til](https://cassey.dev/til). Huzzah! 
